...
export class SkinsService {
  ...
  public getAllSkins(): Observable<Skin[]> {
    return this.http.get<Skin[]>(URL).pipe(
      catchError(this.handleError('getAllSkins', [])),
    );
  }

  /* Get all skins of a weapon using its generated ObjectId*/
  public getSkinsByWeaponObjectId(objectId: string): Observable<Skin[]> {
    return this.http.get<Skin[]>(`${URL}/${objectId}`).pipe(
      catchError(this.handleError('getSkinsByWeaponObjectId', [])),
    );
  }
